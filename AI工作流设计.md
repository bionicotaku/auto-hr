...
- 统一通过 **LangChain + OpenAI Responses API** 发起调用
...
- 某个调用绕过 LangChain 直接手写 OpenAI 请求
...
在 B 页面中，以下两部分构成后续 AI 调用的当前事实输入：
...
其中当前工作描述和当前 rubric 优先级最高。

`重新生成` 不走这套输入规则。  
在 `重新生成` 时，不输入当前工作描述编辑框内容，也不输入当前 rubric 编辑框内容，避免模型刻意参考或规避当前版本；只基于原始输入、历史摘要、最近 5 轮对话和必要上下文重新生成。
...
AI 返回新的工作描述和 rubric，并直接覆盖当前编辑区。
...
- 若当前操作是普通对话或 Agent 模式，则输入当前工作描述和当前 rubric
    
- 若当前操作是 `重新生成`，则不输入当前工作描述和当前 rubric
...
但在点击“完成”时，系统会进入最终定稿流程，由 AI 基于最终内容输出可入库的正式结果。

---

## 6.8 B 页面点击“完成”时的最终定稿调用
...
只要用户在 B 页面点击“完成”，系统都必须再次调用一次 LLM，并显示 loading。

这次调用不是普通对话，而是 **最终定稿调用**。
...
**工作描述与 rubric 生成/编辑 → B 页面点击完成时最终定稿并生成正式评估规范 → Candidate 原始资料临时接收 → Candidate 轻量标准化 → 基于 rubric 的子 agent 逐项分析 → supervisor 汇总总分、summary 与证据点**
